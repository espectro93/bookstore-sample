...
public class OrderController {
    ...
    public OrderDto viewOrder(@Argument String orderId) {
        return OrderDto.createFrom(viewOrder.run(new ViewOrderQuery(new OrderId(orderId))));
    }
    ...
    @MutationMapping
    public OrderDto cancelOrder(@Argument String customerId, @Argument String orderId, @Argument List<PlaceOrderItemDto> placeOrderItems) {
        return OrderDto.createFrom(
                cancelOrder.run(
                        new CancelOrderCommand(new CustomerId(customerId), new OrderId(orderId), placeOrderItems.stream().map(PlaceOrderItemDto::toDomain).toList())
                )
        );
    }
}
